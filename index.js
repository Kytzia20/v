...
+ 'FN:Kytzia-Bot\n' // Nama
+ 'ORG:Kytzia-Bot;\n' // Nama bot
+ 'TEL;type=CELL;type=VOICE;waid=51935684086:+51 935 684 086\n' // Nomor bot
+ 'END:VCARD' 
...
Usa ${prefix}verify para verificarte y poder usar el bot.`
	...
	Lxa.on('chat-update', async (mek) => {
      ...
      const is = budy.slice(0).trim().split(/ +/).shift().toLowerCase()
			mess = {
  wait: 'Espere mmg 😡 ',
  success: 'Todo salio piola puto :D',
  Public: 'Modo privado activado 👩‍🦲',
  ferr: 'Lo siento ocurrio un error, no me funes',
...
case 'menu':
hasil = `        ────────────────
Hola hermosa*${pushname}* usa este comando ${prefix}menu
        ────────────────`
reply(hasil)
        break
}
...
┃ _Ex : ${prefix}Ephoto 25 Kytzia_
┠⊷️ *${prefix}Textpro list*
┃ _Lista de textpro
┠⊷️ *${prefix}Textpro (code + Texto)*
┃ _Ex : ${prefix}Textpro 55 Kytzia_
┗━━━━━━━━━━━━━━━━━━━━
*〘 IMAGENES 〙*
┏━━━━━━━━━━━━━━━━━━━━
┠⊷️ *${prefix}Cogan*
...
//--Convert stiker to image
				case 'toimg':
					...
					exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Kytzia-Bot'})
						fs.unlinkSync(ran)
					})
					break
